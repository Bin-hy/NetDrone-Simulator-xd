...
public class WhipClient
{
    ...
    public IEnumerator Publish(RTCPeerConnection pc, string url, string token)
    {
        ...
        using (var httpClient = new UnityWebRequest(url, "POST"))
        {
            ...
            {
                Debug.LogError($"Request failed with status  {httpClient.responseCode}, result :{httpClient.result}");
                yield return 0;
            }
            ...
            foreach(var item in  httpClient.GetResponseHeaders())
            {
            ...
            Debug.Log($"responseHeaders : {httpClient.GetResponseHeaders()}");
            //etag = httpClient.GetResponseHeaders()["e-tag"]; // TODO 
    ...
    public IEnumerator Tricke()
    {
        ...
        using (UnityWebRequest client = new UnityWebRequest(resourceURL, "PATCH"))
        {
            ...
            //client.SetRequestHeader("If-Match", etag); // TODO
